...
dir.create(glue("{output_path}/raw/"), showWarnings = FALSE, recursive = TRUE)
dir.create(glue("{output_path}/processed/"), showWarnings = FALSE, recursive = TRUE)
...
#and then do the same thing at the sub basin and basin level
dt_fish_results_basin <- dt_fish_results_site |> 
  group_by(SubBasinOrSubZone, Fyear) |> 
  mutate(SubBasinNumUniqIndSpp = n_distinct(Species)) |> 
  unique() |> 
  mutate(SubBasinTotNumFish = sum(SiteTotNumFish),
         SubBasinTotNumNoIFish = sum(SiteTotNumNoIFish),
         SubBasinTotNumAliFish = sum(SiteTotNumAliFish),
         SubBasinTotNumTraFish = sum(SiteTotNumTraFish),
         SubBasinPropTrans = median(PropTrans),
         SubBasinPropAlien = median(PropAlien),
         SubBasinPonis = median(SitePonis, na.rm = T),
         SubBasinPoise = median(SitePoise, na.rm = T)) |> 
  ungroup() |> 
  group_by(BasinOrZone, Fyear) |> 
...
#save data
cond_form_rc_grades(summary_stats_table, glue("{output_path}/main_report_summary_stats"), cols = c(12, 14), method = "Numeric")
...
#save
write_csv(dt_site_summary, glue("{output_path}/appendix_site_summary.csv"))
...
write_csv(dt_species_summary, glue("{output_path}/appendix_species_summary.csv"))
